...
class Flow3:
    ...
    def get_run_option_from_file(self):
        # Read the run option from run.txt
        filename = "run.txt"
        try:
            with open(filename, 'r') as f:
                option_str = f.readline().strip()
            if option_str not in ["1", "2", "3"]:
                self.log(f"{Fore.RED + Style.BRIGHT}Invalid run option in {filename}. Please use 1, 2, or 3.{Style.RESET_ALL}")
                exit(1)
            option = int(option_str)
            proxy_type = (
                "Run With Monosans Proxy" if option == 1 else 
                "Run With Private Proxy" if option == 2 else 
                "Run Without Proxy"
            )
            self.log(f"{Fore.GREEN + Style.BRIGHT}{proxy_type} Selected.{Style.RESET_ALL}")
            return option
        except FileNotFoundError:
            self.log(f"{Fore.RED + Style.BRIGHT}File {filename} Not Found. Please create the file with run option (1, 2, or 3).{Style.RESET_ALL}")
            exit(1)
        except Exception as e:
            self.log(f"{Fore.RED + Style.BRIGHT}Error reading {filename}: {e}{Style.RESET_ALL}")
            exit(1)
    ...
    async def main(self):
        try:
            ...
            # Get run option from run.txt instead of user input
            use_proxy_choice = self.get_run_option_from_file()
